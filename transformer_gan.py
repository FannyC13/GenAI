...
def transformer_generator(latent_dim=100):
    ...
    '''
    Ici on divise en 4 têtes pour explorer en parallèle sur plusieurs couches
    ...
    On normaise pour stabiliser l'entrainement puis on applique ensuite une couche Dense pour introduire de la non-linéarité et transformer les données pour capturer des caractéristiques supplémentaires et plus riches. 
...
def transformer_discriminator(): #(cf cnn_gan.py)
    ...
    x = layers.Dropout(0.3)(x) #Dropout pour éviter le surapprentissage
